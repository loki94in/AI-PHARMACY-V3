...
| Field | Content |
|---|---|
| **What the user saw** | 1. When opening the Pharmarack Cart (`/pharmarack-cart`), users briefly saw a small, outdated/stale cart snapshot before the page abruptly refreshed and morphed into the live full cart.<br>2. When deleting an item from the live cart, the item disappeared optimistically in the UI, but upon refreshing the page or switching away to another page and returning, the deleted item reappeared in the cart. |
| **Root cause** | 1. **Stale Cache Snapshot Flash:** `loadPersistedCartCache` in `frontend/src/pages/PharmarackCart/index.tsx` read items from `localStorage` indefinitely without a Time-To-Live (TTL) expiration check, rendering ancient cart snapshots on mount before network fetch completed.<br>2. **OpenSearch ProductCode Overwrite on Delete:** In `src/routes/pharmarack.ts` (`executeSingleItemDelete`), when `resolvedProductId` was absent (Pharmarack cart items frequently return `ProductId: null`), the backend performed OpenSearch product enrichment. If OpenSearch found a product from an alternate distributor/store, it overwrote `resolvedProductCode` with the mismatched product code, causing upstream `DeleteUserCartDetailByStoreIdV2` to receive an invalid code for that `store_id`.<br>3. **Blind HTTP 200 Trust:** Upstream Pharmarack API `DeleteUserCartDetailByStoreIdV2` returns HTTP status 200 even when a product code does not exist in the store's cart, but returns the updated `IList`. The backend blindly trusted the HTTP status without inspecting `IList` to confirm the item was removed.<br>4. **Component Unmount on Navigation:** `KeepAliveOutlet.tsx` did not include `'/pharmarack-cart'` in `HIGH_PRIORITY_PATHS`. Navigating to another page unmounted the component, terminating pending in-flight deletion buffer queues and clearing `pendingDeleteKeysRef`. Subsequent re-fetch or page visits restored the un-deleted item. |
| **How it was fixed** | 1. **Preserve Pharmarack Cart in KeepAlive:** Added `'/pharmarack-cart'` to `HIGH_PRIORITY_PATHS` in `frontend/src/lib/keepAlive/KeepAliveOutlet.tsx` and `WARMUP_PATHS` in `frontend/src/App.tsx` so the cart remains mounted in the DOM across page switches and background delete queues complete uninterrupted.<br>2. **Module-Level Delete Queue:** Moved delete task queues and pending deletion keys (`globalDeleteQueue`, `globalPendingDeleteKeys`, `globalIsProcessingDeleteQueue`) to module-level scope in `PharmarackCart/index.tsx` so deletions survive any re-render or navigation hand-off.<br>3. **Surgical OpenSearch Delete Bypassing:** In `src/routes/pharmarack.ts`, prevented OpenSearch enrichment from overwriting valid `resolvedProductCode` values provided directly from the live cart item, ensuring the exact code is submitted to Pharmarack.<br>4. **Upstream Cart Deletion Verification:** Verified that `secJson.IList` does not contain the deleted `ProductCode` before considering the operation successful, avoiding false positive 200 confirmations.<br>5. **Stale Cache Expiration:** Added a 15-minute TTL (`CART_CACHE_MAX_AGE_MS`) to `loadPersistedCartCache`. Stale items older than 15 minutes are discarded so the page cleanly shows skeleton loading instead of flashing ancient cart state.<br>6. **Reduced Delete Buffer:** Decreased `PHARMARACK_DELETE_BUFFER_MS` from 5000ms to 800ms for fast upstream dispatch while retaining batching efficiency. |
| **Priority** | P1 |
| **What not to touch** | Distributor contact resolution, auto-push cart sync coordinator, token refresh scheduler heartbeat, and POS/Order creation workflows. |
| **Verified by** | `tests/pharmarackCartDelete.test.ts` (2/2 PASS); `tests/pharmarackCartItemVisibility.test.ts` (4/4 PASS); `npm --prefix frontend run build` (PASS); `npm run guardrails` (PASS, 0 violations); Auto-Knowledge Graph updated. |

### [Fixed] P2-08 — SQLite Error in distributorSync and pharmarack routes: Missing delivery_boy_id column & Header Pill ⚠️6 Unmapped Warning

| Field | Content |
|---|---|
| **What the user saw** | 1. Background worker and API logs reported `[distributorSync] Failed to sync pharmarack_distributor_mappings: [Error: SQLITE_ERROR: table pharmarack_distributor_mappings has no column named delivery_boy_id]` and `Failed to fetch distributor mappings: [Error: SQLITE_ERROR: no such column: d.delivery_boy_id]`, and `[NODE-CRON] [WARN] missed execution` due to event loop starvation from repetitive query errors.<br>2. In the header `<LiveCartCountdownPill>`, a red badge `⚠️6` warned that 6 distributors were missing phone numbers, even after the user had previously entered/saved phone numbers on `/pharmarack-cart?filter=unmapped`. |
| **Root cause** | 1. `src/database.ts` defined column migrations for `distributors.delivery_boy_id` and `pharmarack_distributor_mappings.delivery_boy_id` at schema v60, but `CURRENT_SCHEMA_VERSION` remained at 60. On boot, existing databases already had `schema_version = 60`, which triggered the fast-boot path and bypassed all DDL migrations without ensuring `delivery_boy_id` existed on those tables. In addition, `distributorSyncHelper.ts` relied on `CREATE TABLE IF NOT EXISTS`, which does not add missing columns to pre-existing tables.<br>2. When users saved missing distributor numbers in the frontend UI modal, optimistic in-memory state updated, but the background write to `POST /api/pharmarack/distributor-mappings` crashed silently due to the missing column, leaving the mappings unsaved in SQLite.<br>3. In `GET /api/pharmarack/live-cart-summary` (`src/routes/pharmarack.ts`), the backend strictly checked exact string equality `LOWER(TRIM(m.store_name)) = LOWER(TRIM(d.name))` without calling `resolveDistributorContact` for fuzzy/normalized variants (e.g. `CM Distributors` vs `CM DISTRIBUTOR`, `BETTER LIFE PHARMA` vs `BETTER LIFE`). |
| **How it was fixed** | 1. Bumped `CURRENT_SCHEMA_VERSION` to 61 in `src/database.ts`.<br>2. Added defensive column checks and ALTER TABLE migrations for `distributors.delivery_boy_id` and `pharmarack_distributor_mappings.delivery_boy_id` / `store_id` in both the fast-boot path and the schema upgrade path in `src/database.ts`.<br>3. Added PRAGMA table_info column verification and self-healing in `src/utils/distributorSyncHelper.ts`.<br>4. Integrated `resolveDistributorContact(db, dist.storeName)` fallback in `GET /api/pharmarack/live-cart-summary` so store names with minor spelling/format variations are automatically matched and persisted.<br>5. Added minimum token length checks (>=4) in Priority C substring matching across backend and frontend to prevent 2-letter abbreviations from false-matching unrelated words.<br>6. Confirmed runtime database migrated to v61 and verified `GET /api/pharmarack/live-cart-summary` now reports `missingDistributorsCount: 0`. |
| **Priority** | P2 |
| **What not to touch** | Delivery boy assignment logic (`/api/dispatch`), distributor notification triggers, and keep-alive caching. |
| **Verified by** | `tests/distributorNotification.test.ts` (9/9 PASS); `data/app.db` schema inspection (PASS, delivery_boy_id present on both tables); `GET /api/pharmarack/distributor-mappings` (PASS, 37 mappings loaded); `GET /api/pharmarack/live-cart-summary` (PASS, missingDistributorsCount: 0); `npm run guardrails` PASS. |
