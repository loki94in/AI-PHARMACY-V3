  ...
  try {
    ...
    const result = await db.run(
      ...
      [
        ...
        tb_medicine ? 1 : 0,
        allow_loose_sale !== undefined 
          ? (allow_loose_sale ? 1 : 0) 
          : ((`${packaging || ''} ${adjustedName || ''}`.toLowerCase().match(/bottle|syrup|suspension|drop|inj|liquid|lotion|spray|ointment|gel|cream|\bml\b/i)) ? 0 : 1),
