...
- **Truthful OOS vs search-failure (bug fix, 2026-09-20)**: `proceedWithConfirmedProcurement` (whatsappIntentService.ts) tracks `searchFailed` separately from "genuinely zero in-stock candidates". A `performPharmarackSearch` timeout/`connection_error` gets ONE retry, then falls to the offline catalog; if still empty, the customer/owner messages say search is temporarily unavailable ("Search Failed — Needs Manual Check") instead of the false "All Distributors OOS" claim. Root cause: `performPharmarackSearch`'s live OpenSearch call previously silently returned `{status:'ok', items:[]}` on ANY failure (5s timeout too short + masked errors in routes/pharmarack.ts), which this flow's `status === 'ok'` check then read as confirmed zero stock — real customers were told medicines were unavailable everywhere when they were actually in stock (SO-TMSA-3/4, 2026-09-20 incident). The live search timeout was raised 5s→8s (retry leg 4s→5s) and the route no longer masks live-call failures as success. Never reintroduce a `catch { silently treat as empty }` pattern around performPharmarackSearch without checking `status` first.
