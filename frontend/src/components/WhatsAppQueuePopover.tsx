      ...
      >
        ...
        <div className="p-2.5 flex items-center justify-between gap-3 text-xs">
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            {(item.status === 'skipped_not_on_whatsapp' || item.status === 'skipped_invalid_phone') && (
              <span className="text-[10px] px-2 py-0.5 rounded-full font-bold bg-amber-500/20 text-amber-300 border border-amber-500/30 flex items-center gap-1 font-mono">
                <AlertTriangle size={10} /> {item.status === 'skipped_not_on_whatsapp' ? 'Not on WhatsApp' : 'Invalid Phone'}
              </span>
            )}
